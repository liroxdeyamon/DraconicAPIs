...
CHARACTERS.textToEntriesByText = text =>
    entriesFromField(text, ["letter"]);

CHARACTERS.textToEntriesByRom = text =>
    entriesFromField(text, ["letter_rom"]);

CHARACTERS.textToEntriesByDiscord = text =>
    entriesFromField(text, ["letter_discord"]);

CHARACTERS.textToEntriesByGlyph = text =>
    entriesFromField(text, ["letter_glyph"]);

CHARACTERS.entriesToText = (entries, ignore_optional = false) =>
    entries
...
CHARACTERS.entriesToDiscord = (entries, ignore_optional = false) =>
  entries
    .filter(e => !(ignore_optional && e.prop?.includes(REG.OPTIONAL)))
    .map(e => e.letter_discord || "")
    .join("");

CHARACTERS.entriesToGlyphs = (entries, ignore_optional = false) =>
  entries
    .filter(e => !(ignore_optional && e.prop?.includes(REG.OPTIONAL)))
